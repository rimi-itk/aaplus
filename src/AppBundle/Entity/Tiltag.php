...
abstract class Tiltag
{
    ...
    {
        if (null !== $this->tilvalgtAfMagistrat) {
            return $this->tilvalgtAfMagistrat;
        }

        if (null !== $this->tilvalgtAfAaPlus) {
            return $this->tilvalgtAfAaPlus;
        }

        if (null !== $this->tilvalgtAfRaadgiver) {
     ...
     *
     * @param int $faktorForReinvesteringer
     *
     * @return Tiltag
     */
    ...
    public function calculate()
    {
        ...
        if (null !== ($value = $this->calculateBesparelseDriftOgVedligeholdelse())) {
            $this->besparelseDriftOgVedligeholdelse = $value;
        }
        // This may be computed, may be an input
        if (null !== ($value = $this->calculateLevetid())) {
            $this->levetid = $value;
        }
     ...
     */
    public function setRisikovurderingOekonomiskKompenseringIftInvesteringFaktor(
        $risikovurderingOekonomiskKompenseringIftInvesteringFaktor
    ) {
    ...
    public function calculateSavingsForYear($year)
    {
        ...
        $besparelse = // $this->getIndtaegtSalgAfEnergibesparelse()
            ...
            + ($this->getBesparelseStrafafkoelingsafgift() + $this->getBesparelseDriftOgVedligeholdelse()) * pow(
                1 + $this->rapport->getInflation(),
                $year
            );
     ...
     *
     * @param bool $tilvalgtAfMagistrat
     *
     * @return Tiltag
     */
     ...
     *
     * @param bool $tilvalgtAfRaadgiver
     *
     * @return Tiltag
     */
    ...
    {
        if (0 === $this->getRapport()->getLobetid() || 0 === $this->levetid) {
            return 0;
        }
    ...
    protected function calculateCashFlow($numberOfYears, $yderligereBesparelseKrAar = 0)
    {
        ...
        $aaplusinvestering = (float) ($this->aaplusInvestering);
        $varmebesparelseGUF = (float) ($this->varmebesparelseGUF);
        $varmebesparelseGAF = (float) ($this->varmebesparelseGAF);
        $elbesparelse = (float) ($this->elbesparelse);
        $vandbesparelse = (float) ($this->vandbesparelse);
        $besparelseStrafafkoelingsafgift = (float) ($this->besparelseStrafafkoelingsafgift);
        $besparelseDriftOgVedligeholdelse = (float) ($this->besparelseDriftOgVedligeholdelse);
        $scrapvaerdi = (float) ($this->scrapvaerdi);
        ...
        for ($year = 1; $year <= $numberOfYears; ++$year) {
            ...
            if (1 === $year) {
                ...
                if (15 === $numberOfYears && $year === $numberOfYears) {
                    $value += $scrapvaerdi;
                }
            }
